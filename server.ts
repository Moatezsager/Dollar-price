      ...
      try {
        ...
        for (const block of messageBlocks) {
          ...
          if (textMatch && timeMatch) {
            ...
            // Use flexible regexes to capture various formats. Max 20.0 to allow real market spikes.
            // Matches: "الدولار: 7.30", "دولار 7.3", "USD = 7.305", etc.
            extract(/(?:الدولار كاش|دولار كاش|الدولار|دولار|usd|🇺🇸)\s*(?:=|:|:|بـ|-|\s)\s*(\d{1,2}(?:\.\d{1,3})?)/i, 'USD', 5.0, 20.0);
            extract(/(?:صكوك|بنوك|الدولار بنوك|صك|الجمهورية|التجارة|الوحدة|الامان)\s*(?:=|:|:|بـ|-|\s)\s*(\d{1,2}(?:\.\d{1,3})?)/i, 'USD_CHECKS', 5.0, 20.0);
            extract(/(?:يورو|اليورو|eur|🇪🇺)\s*(?:=|:|:|بـ|-|\s)\s*(\d{1,2}(?:\.\d{1,3})?)/i, 'EUR', 5.0, 20.0);
            extract(/(?:باوند|الباوند|gbp|🇬🇧)\s*(?:=|:|:|بـ|-|\s)\s*(\d{1,2}(?:\.\d{1,3})?)/i, 'GBP', 5.0, 20.0);
            extract(/(?:كسر الذهب عيار 18|كسر الذهب|ذهب)\s*(?:=|:|:|بـ|-|\s)\s*(\d{2,4}(?:\.\d+)?)/i, 'GOLD', 100, 5000);
            extract(/(?:دولار حوالات تركيا|حوالات تركيا)\s*(?:=|:|:|بـ|-|\s)\s*(\d{1,2}(?:\.\d{1,3})?)/i, 'USD_TR', 5.0, 20.0);
            extract(/(?:دولار حوالات دبي|حوالات دبي)\s*(?:=|:|:|بـ|-|\s)\s*(\d{1,2}(?:\.\d{1,3})?)/i, 'USD_AE', 5.0, 20.0);
          }
        }
      } catch (err) {
