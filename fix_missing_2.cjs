...
// Find where to insert it: after the cron job.
const searchStr = `await broadcastRateChanges(delayedUpdates, false, 'all');\n  }\n});`;

const insertIndex = c.indexOf(searchStr);
if (insertIndex !== -1) {
  c = c.substring(0, insertIndex + searchStr.length) + "\n" + missingCode + c.substring(insertIndex + searchStr.length);
