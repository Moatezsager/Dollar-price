...
const targetStr = `        if (history.length > 5000) history.shift();
      }
      return anyChanged;
    }
    return false;
...
const replacementStr = `        if (history.length > 5000) history.shift();
      }
...
if (code.includes(targetStr)) {
    ...
    // Now add the watchdog interval around line 5060 (where Memory Monitor is)
  ...
  // Memory Monitor`;
    ...
    console.log("Successfully patched watchdog.");
} else {
    console.log("Target string not found for watchdog.");
}
