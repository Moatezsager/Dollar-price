...
# find the function string
func_str = """async function broadcastOfficialRates(isTest: boolean = false) {
...
# I'll use a regex that deletes it. The safest way is to just delete the exact string!
escaped_func = re.escape(func_str)
content = re.sub(escaped_func, '', content)

# Now inject it before fetchOfficialRates (line 1097 is fetchOfficialRates)
# I will just place it right after broadcastRateChanges (which is around line 1493)
# Actually, I'll place it right above fetchOfficialRates
insert_marker = "async function fetchOfficialRates(): Promise<boolean> {"
content = content.replace(insert_marker, func_str + "\n\n" + insert_marker)
