...
const fetchOfficialTarget = `async function fetchOfficialRates(): Promise<boolean> {
...
const fetchOfficialReplacement = `let lastOfficialFetchDate = "";
...
const fetchOfficialTarget2 = `    if (anyChanged) {
...
const fetchOfficialReplacement2 = `    if (anyChanged) {
      rates.official = { ...rates.official, ...cblRates };
      rates.parallel.OFFICIAL_USD = rates.official.USD;
      rates.lastChanged.parallel.OFFICIAL_USD = new Date().toISOString();
      console.log(\`[Official] Rates updated via CBL Scraper\`);
...
if (server.includes(fetchOfficialTarget) && server.includes(fetchOfficialTarget2)) {
  server = server.replace(fetchOfficialTarget, fetchOfficialReplacement);
  server = server.replace(fetchOfficialTarget2, fetchOfficialReplacement2);
